...
from pipeline import (
    ...
    get_debiased_predictions, remediate_with_reaudit, _state
)
...
async def upload_dataset(
    ...
    positive_label: str | None = Form(None),
    fairness_criterion: str = Form("equal_opportunity"),
    difference_bound: float = Form(0.05),
    ...
    if fairness_criterion not in {"equal_opportunity", "demographic_parity"}:
        raise HTTPException(
            status_code=400,
            detail="fairness_criterion must be 'equal_opportunity' or 'demographic_parity'.",
        )
    if not 0 <= difference_bound <= 1:
        raise HTTPException(status_code=400, detail="difference_bound must be between 0 and 1.")
...
async def remediate(
    ...
    remediation_method: str = Form("reweighing"),
    repair_level: float = Form(1.0),
    authorization: str | None = Header(None),
    x_user_id: str | None = Header(None, alias="X-User-Id"),
):
    """
    ...
    required_cols = list(_state.get("feature_cols") or []) + [
        _state["sensitive_col"],
        _state["target_col"],
    ]
    ...
    if remediation_method not in {"reweighing", "disparate_impact_remover"}:
        raise HTTPException(
            status_code=400,
            detail="remediation_method must be 'reweighing' or 'disparate_impact_remover'.",
        )
    if not 0 <= repair_level <= 1:
        raise HTTPException(status_code=400, detail="repair_level must be between 0 and 1.")

    # Store raw_df in state so remediation helpers can export this request's rows.
    _state["raw_df"] = df.copy()

    try:
        remediation = remediate_with_reaudit(
            df,
    ...
    export_df = remediation.dataframe

    # Add sensitive col back
    summary = {
        "method": remediation.method,
        "repair_level": round(remediation.repair_level, 3),
        "weight_power": round(remediation.weight_power, 3),
        "attempts": remediation.attempts,
        "labels_preserved": True,
        "rows_exported": int(len(export_df)),
        "original_di_ratio": round(remediation.original_di_ratio, 3),
        "verified_di_ratio_after_retraining": round(
            remediation.verified_di_ratio_after_retraining, 3
        ),
        "debiased_di_ratio": round(remediation.verified_di_ratio_after_retraining, 3),
        "verified_metrics": remediation.verified_metrics,
    }
    try:
        save_remediation_summary(audit_id, remediation.verified_di_ratio_after_retraining)
    except DatabaseOperationError:
        pass
